...
@if (Auth::check())
    @if(Auth::user()->rol=='admin')
        ...
        <a href="{{ route('races.destroy', $race->id) }}" class="btn btn-danger">Eliminar raza</a>
    @endif
@endif
@if ($race->descripcion=='')
@else
    ...
    <div class="row">
        ...
        @forelse ($race->subRaces as $subrace)
            <div class="col-12">
                <div class="card">
                    <div class="row">
                        <div class="col-8">{{$subrace->nombre}}</div>
                        @if (Auth::check())
                        @if(Auth::user()->rol=='admin')
                        <div class="col-2">
                            <form action="{{route('users.destroy', $subrace->id)}}" method="post">
                                @csrf
                                @method('delete')
